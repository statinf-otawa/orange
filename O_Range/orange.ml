	...
	let iAmNotNul = !estNulEng = false in
		...
		if iAmExact   then 
			begin
				...
				let nb = expressionEvalueeToExpression (evalexpression  (Diff ((*myBorne*) !borneMaxAux, ConstInt ("1"))))  in(*max ???*)
...
let evaluerFonctionsDuDoc  doc=	
	...
	begin
		...
		abGlobales :=  evalStore 	(new_instBEGIN !listeDesInstGlobales) []	;
