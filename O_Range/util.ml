...
*)
  if l = [] then 
    ...
    match a with
      ASSIGN_SIMPLE (s, e) -> 
        if s = v then e else rechercheAffectVDsListeAS v (*index*) suite
...
let (callsListEvalAssignVar: 
    (string * (string list * string list)) list ref) = ref []
  ...
  if assignList = [] then []
    ...
    let firstAs =
        ...
        VAR ( id, exp,_,_)  ->  
          if List.mem_assoc id !listeAssosPtrNameType then  
             (match exp with
                ...
                (* if a field of struct is assigned it is either a compound
                   either multiple not inportant*)
            | _ -> [id])
          else [id]
      | TAB ( id, _, _,_,_)
      | MEMASSIGN ( id, _, _,_,_) ->   
          ...
          in 
            if List.mem_assoc fid !listeAssosPtrNameType then ["*"^fid]  
            else [id]  
      ...
      | APPEL (num, e, nom, s, CORPS c,v,r,_) -> 
          let listVar = 
            if List.mem_assoc nom !callsListEvalAssignVar then 
              (let (la,_) = List.assoc nom !callsListEvalAssignVar in la)
            else assignVar [c]
          in
          let globalPtr = 
                List.filter(fun x-> List.mem x !alreadyAffectedGlobales) listVar
          in
      ...
      | APPEL (num, e, nom, s, ABSSTORE a,v,r,_) ->
          let globalPtr = List.filter(
            fun x -> List.mem x !alreadyAffectedGlobales
            ) (getAllVARAssign a)
          in
            let sorties = (match s with BEGIN(sss)-> sss |_->[]) in
              let ass = List.map(
...
let isLoopOrIFId x =
  let resb = 
    if (String.length x > 4) then begin
      let var4 = (String.sub x 0 4) in
      let var3 = (String.sub x 0 3) in
      if var3 = "ET-" || var3 = "EF-" ||  var3 = "IF-" 
                || var3 = "tN-" || var4 = "max-" || var4 = "tni-" 
        then true else false
      end else if (String.length x > 3) then 
        if (String.sub x 0 3) = "ET-" || (String.sub x  0 3) = "EF-"
           || (String.sub x  0 3) = "IF-" || (String.sub x  0 3) = "tN-" then 
          true 
        else 
          false 
      else 
  ...
  match exp with
    BINARY (op, _, _) ->
      (match op with 
        ...
        MOD_ASSIGN | BOR_ASSIGN | XOR_ASSIGN | SHL_ASSIGN | SHR_ASSIGN -> true
       | _ -> false)
  | UNARY (op, _) -> 
      (match op with
...
let rec hasPtrArrayBoundConditionExp e =
  ...
  | BINARY (_, exp1, exp2) ->
      ...
      if b1 && b2 = false then 
        (b1,e1, iv1, exp1)
      else if b1 = false && b2 then 
        (b2,e2, iv2, exp2 ) 
      else 
        (false, "", true, NOTHING) 
  | VARIABLE id -> 
      if (String.length id > 19) then
        if (String.sub id  0 19) = "getvarTailleTabMax_" then begin
        ...
        (*  Printf.printf "dans hasPtrArrayBoundConditionExp, trouve %s type : \n" var;
          ...
          (* Printf.printf "dans hasPtrArrayBoundConditionExp, autre\n" ;*)
          (false, "",true,NOTHING) 
        end
      else
        (false, "", true, NOTHING) 
    | CALL(VARIABLE("getvarTailleTabMax_"), [e]) -> 
        (*Printf.printf "dans hasPtrArrayBoundConditionExp, autre call\n" ; *)
        (true, "", false, e)   
    | CALL(_, [e]) ->
        (*Printf.printf "dans hasPtrArrayBoundConditionExp, autre call 2\n" ; *)
        hasPtrArrayBoundConditionExp e 
    | _ -> (false, "", true, NOTHING) 

let hasPtrArrayBoundCondition e =
  match e with
    MULTIPLE -> (false, "", true, NOTHING)
  | EXP (e) -> hasPtrArrayBoundConditionExp e 
...
let rec containtNothing e =
  ...
  | EXPR_LINE (exp, _, _) -> 
      containtNothing exp
  | BINARY (_, exp1, exp2) -> 
      containtNothing exp1 || containtNothing exp2
  | QUESTION (exp1, exp2, exp3) -> 
      containtNothing exp1 || containtNothing exp2|| containtNothing exp3
  | CALL (exp, args) ->
      containtNothing exp||listnothing args
  | COMMA exps -> 
      listnothing exps
  | CONSTANT _ 
  | VARIABLE _ 
  ...
  | GNU_BODY (_, _) -> 
      false
  | INDEX (exp, idx) ->
      containtNothing exp || containtNothing idx
and listnothing l = 
  if l = [] then 
    false 
  else 
    containtNothing (List.hd l) || listnothing (List.tl l) 


let estMultipleDef v (*index*) l =
  match rechercheAffectVDsListeAS v (*index*) l with 
    MULTIPLE -> true
  | EXP (e) -> false

let estDef v (*index*) l = 
  match rechercheAffectVDsListeAS v (*index*) l with 
    MULTIPLE -> true 
  | EXP (e) -> if e = NOTHING then false else true
...
let rec existAffectVDsListeAS v (*index*) l =
  ...
  if l = [] then 
    ...
    match a with
      ASSIGN_SIMPLE (s, e) -> 
        if s = v then true else existAffectVDsListeAS v (*index*) suite
    | ASSIGN_DOUBLE (s,e1, e2) ->   
