...
let rec removeSpecifieur st intType=
  ...
  if length > 1 then begin
    ...
    let sub = (String.sub st  0 (length-1)) in
      if intType then
        if  (lCar == 'l' || lCar == 'L' || lCar == 'u' || lCar == 'U') then
          (removeSpecifieur (sub) intType)
        else 
          st
      else if (lCar == 'l' || lCar == 'L' || lCar == 'f' || lCar == 'F') then
        removeSpecifieur (sub) intType
      else 
        st
  end else
...
let rec getListIntVar l n globalesVar =
  if l = [] then
    ...
    let isInt = match getBaseType typ with 
      INT_TYPE -> true
    |_-> false in
    let (isOk,nn) =
      if isInt then begin
        if (String.length var > 4) && (String.sub var  0 4) = "bIt-" then
          (false,n)
        else if (String.length var > 5) && ((String.sub var  0 5) = "__tmp" ||  (String.sub var  0 5) = "__tag") then
          (false,n) else (true, n+1)
      end else 
        (false,n)
    in
    let (g, o)= (getListIntVar next nn globalesVar) in
    if isOk then 
      if List.mem var globalesVar then
        (List.append [(var, n)] g,o)
      else
        (g, List.append [(var, n)] o)
    else
      (g, o)
  end
...
match exprEvaluee with
  ...
  | Var (s) -> VARIABLE(s)
  | Sum (f, g) ->
      let exp1 = expressionEvalueeToExpression f in
  ...
  | PartieEntiereSup (e)->
      let exp1 = expressionEvalueeToExpression e in
  ...
  | Sygma (var,min,max,e) ->
      let exp1 = expressionEvalueeToExpression max in
  ...
  | Eq1 (e1) ->
      let exp1 = expressionEvalueeToExpression e1 in
...
let estNoComp expr = 
  ...
  | Max (_,_,_,_)-> 
      true 
  | ConstInt(i) ->
      if i = "" || is_integer i =false then true else false
  | ConstFloat (f) ->
      if f = "" || is_float f =false  then true else false 
  | _-> 
      false
...
let rec print_expTerm exprEvaluee =
  ...
  | RConstFloat (f) -> 
      Printf.printf " %g" f
  | NOCOMP ->
      Printf.printf " NOCOMP"
  | Boolean(b) -> 
      if b = true then Printf.printf " true"  else Printf.printf " false"
  | Var (s) -> 
      Printf.printf " %s" s
  | Sum (f, g) ->
      Printf.printf "(";
  ...
  | Puis (f, g) ->
      ...
      Printf.printf ")"
  | PartieEntiereSup (e) ->
      Printf.printf "PartieEntiereSup (";
      print_expTerm e;
      Printf.printf ")"
  | PartieEntiereInf (e) ->
      Printf.printf "PartieEntiereInf (";
      print_expTerm e;
      Printf.printf ")"
  | Log (e) ->
      Printf.printf "Log (";
  ...
  | Eq1 (e1)->
      Printf.printf " Val TEST ";
      print_expTerm e1;
      Printf.printf " else  "
  | Maximum (f, g) ->
      Printf.printf " maximum( ";
      print_expTerm f;
      Printf.printf ", ";
      print_expTerm g;
      Printf.printf " ) "
  | Minimum (f, g) ->
      Printf.printf " minimum( ";
      print_expTerm f;
      Printf.printf ", ";
      print_expTerm g;
      Printf.printf " ) "
...
let rec epsIsonlyVar exp =
  match exp with
    ...
    | RConstFloat (_) -> true
    | Var (s) -> 
        if s = "EPSILON" || s = "EPSILONINT" then true else false (*MARQUE*)
    | Sum (f, g)
    | Shl (f, g)
    | Shr (f, g)
    | Diff (f, g)
...
let estUn exp =
  ...
  | ConstFloat (i) -> if is_float i then (float_of_string i) > 1.0 else false
  | RConstFloat (i) -> i > 1.0
  | _-> false
...
let rec getIndexValueOfTab val1 expsc =
  if expsc = [] then 
    ...
    if val1 = 0 then 
      ...
      getIndexValueOfTab (val1 -1) next
  end
...
let rec consArrayFromPtrExp exp arrayName =
  ...
  match valsi with
    UNARY (op, exp) ->
      (match op with
         MINUS 
       | PLUS-> let varOfExp = (listeDesVarsDeExpSeules exp) in
                  if List.mem arrayName varOfExp = false then 
                    (exp, false)
                  else 
                    (NOTHING, false)
       | ADDROF ->  
            (* jz: unary addrof variable without index and [] *)
            (NOTHING, false)
       | _ -> (NOTHING, false))
  | BINARY (op, exp1, exp2) ->
      let varOfExp1 = (listeDesVarsDeExpSeules exp1) in
      let varOfExp2 = (listeDesVarsDeExpSeules exp2) in
      if List.mem arrayName varOfExp1 = false then begin
        let (resexp2, hasArray) = consArrayFromPtrExp exp2 arrayName in
        if hasArray then
            if resexp2 = NOTHING then 
              (exp1, true) 
            else 
              (BINARY(op, exp1, resexp2), true)
        else
          (exp2, false)
      end else begin
        if List.mem arrayName varOfExp2 = false then begin
          let (resexp1, hasArray) = consArrayFromPtrExp exp1 arrayName in
          if hasArray then
            if resexp1 = NOTHING then
              (exp2, true) 
            else 
              (* jz: added a special case here to get rid of different results
                     for the following piece of code, where the two statements
                     should have the same semantics:
                 typedef struct { int a, int b } s;
                 s tab[5];
                 int * s = &tab[0]; should be same result as int * s = &tab; 
                 the latter yields the following index expression: EXP (UNARY (ADDROF, VARIABLE "stab"))
                 whereas the other boils down to BINARY (ADD, CONSTANT (CONST_INT "0"), CONSTANT (CONST_INT "0")).
                 there might be a better way to implement this fix, but it works and there should
                 not be other special cases; *)
              if op = ADD && resexp1 = CONSTANT (CONST_INT "0") && exp2 = CONSTANT (CONST_INT "0") then
                (NOTHING, false)   (* &tab is same as &tab[0], but only &tab yields analysis results, e.g. executed attributes *)
              else
                (BINARY(op, resexp1, exp2), true)
          else (exp1, false)
        end else 
          (valsi,false)
      end
  ...
  | VARIABLE name -> 
      if name = arrayName then 
        ((CONSTANT (CONST_INT "0")), true)
      else if (String.length name > 4) then
        if (String.sub name 0 4) = "bIt-" then
          (exp, false) 
        else (NOTHING, false)
      else (NOTHING, false) 
      (* sinon pb si ptr=tab+k ; k=k+1; *ptr=... donc pour le moment que constante*)
  | INDEX (_, _) ->
    (*Printf.printf"actuellement  non traitée index\n";*)(* actuellement  non traitée *)
      let (tab,lidx) = analyseArray valsi []  in
        if (List.mem_assoc tab !listAssosArrayIDsize) then begin
          ...
          let (res, isok) =
            (match size with
             ...
             | SARRAY (_) ->
                 consArrayFromPtrExp 
                        (BINARY (ADD, VARIABLE(tab), List.hd lidx)) arrayName
             | MSARRAY (lsize) ->
                 let index = prodListSizeIndex lsize lidx in
                 consArrayFromPtrExp 
                       (BINARY (ADD, VARIABLE(tab), index)) arrayName ) in
                       (* (NOTHING, false)in*)
  ...
  | _ ->
    (*Printf.printf"actuellement  non traitée\n";*)(* actuellement  non traitée *)
    (NOTHING, false)
...
and getArrayAssign x index assign =
  ...
  Printf.printf "<- \n" ; print_expression (expVaToExp assign)  0; new_line(); 
  flush() ;space(); flush(); new_line(); flush();*)
  let (tab1,_, _) =getArrayNameOfexp (expVaToExp index) in
    if tab1 != "" then begin
      ...
      let (indexexp , isok) = consArrayFromPtrExp (expVaToExp index )  tab1 in
        if isok then begin
        (*let resindex = expressionEvalueeToExpression( calculer (EXP(indexexp))  
        !infoaffichNull [] 1) in*)
        (*print_expression  (INDEX(VARIABLE(tab1), resindex)) 0; new_line(); 
        flush() ;space(); flush(); new_line(); flush();
        Printf.printf "<- \n" ;print_expression (expVaToExp assign) 0; new_line(); 
        flush() ;space flush; new_line(); flush();*)
  ...
  if tab1 != "" then begin
    let (indexexp, isok) = consArrayFromPtrExp (expVaToExp index ) tab1 in
      if indexexp != NOTHING then 
        (tab1, (EXP(indexexp)), true)
      else (tab1, MULTIPLE, true)
  end else 
    (x, index, false)
...
and isBoolFalse val1 =
  if val1 = Boolean (false) || val1 = ConstInt ("0") 
     || val1 = ConstFloat("0.0") || val1 = RConstFloat(0.0) then 
    true 
  else 
    false 
  ...
  match expressionVA with
      ...
      | UNARY (op, exp) ->
          begin match op with
            MINUS ->
                ...
                if estNoComp val1 then 
                  ...
                  evalexpression (Diff (ConstInt ("0"), val1)) (*voir*)
          | PLUS ->
              let val1 = calculer (EXP exp) ia l sign in
              if estNoComp val1 then 
                NOCOMP 
              else  
                evalexpression val1 (*voir*)
          | NOT ->
              ...
              if estNoComp val1 then 
                ...
                if val1 = Boolean(true) || val1 = ConstInt ("1")
                   || val1 = ConstFloat("1.0") || val1 = RConstFloat(1.0) then
                  Boolean(false)
                else
                  if val1 = Boolean (false) || val1 = ConstInt("0")
                     || val1 = ConstFloat("0.0") || val1 = RConstFloat(0.0) then
                    Boolean(true) else NOCOMP (*neg*)
          | BNOT ->   NOCOMP
          | MEMOF -> 
              (*Printf.printf" calculer MEMOF\n";print_expression exp 0; new_line();*)
              (match exp with 
                UNARY(ADDROF, exp2) ->   
          ...
          | PREINCR -> 
              ...
              if estNoComp val1 then 
                ...
                evalexpression (Sum (val1, ConstInt ("1")))
          | PREDECR -> 
              let val1 = calculer (EXP exp) ia l sign in 
              if estNoComp val1 then 
                NOCOMP 
              else 
                evalexpression (Diff (val1, ConstInt ("1")))
          | POSINCR ->
              let val1 = calculer (EXP exp) ia l sign in 
              if estNoComp val1 then 
                NOCOMP 
    ...
    | BINARY (op, exp1, exp2) ->
        ...
        let (unique, var) =
          if listeDesVar = [] then 
            (false, "") 
          else if List.tl listeDesVar = [] then 
            (true, List.hd listeDesVar) 
          else (false, "")
        in
        (*if !vDEBUG then
          begin
        ...
        let isOKAll = 
          if (estDefVal1 = false && estDefVal2 = false)
              || (hasSETCALLval1 = false && hasSETCALLval2 = false) 
              || (isOkval1 && isOkval2) then 
            true 
          else
            false 
        in
        ...
        if multop then begin
          let (val22,estPosVal22) = 
            if isOkval2 then 
              (val2, estPosVal2) 
            else
        ...
        let oksign = ((estPosVal22 = estPosVal2) || isOkval1) 
                      && ((estPosVal11 = estPosVal1 (* || estNul val1 || estNul val11*))
                      ||isOkval2) 
        in
...
and eqindex i j =
  ...
  begin
    ...
    if estNoComp i1 || estNoComp ii1 then 
      false (*en fait en ne peut pas répondre*)
    else 
      if estNul (evalexpression (Diff (i1, ii1 ))) then begin
        (*Printf.printf " index egaux \n" ;*) true
      end else
        false
  end
...
let listeDesVarDependITitcour = ref [] (* as changed during fixed point operator*)
  ...
  if l = [] then 
    ...
    let suite = List.tl l in
      ...
      | ASSIGN_DOUBLE (s, e1, e2) ->
          if s = v (*and il faut évaluer les 2 expression index = e1*) then begin
            if !vDEBUG then begin 
              Printf.eprintf "tableau avec index à terminer\n";(* afficherAS a *) 
            end;
            e2
          end else  
            rechercheAffectVDsListeAS v (*index*) suite
      | ASSIGN_MEM (s, e1, e2)  ->
          if s = v (*and il faut évaluer les 2 expression index = e1*) then begin
            if !vDEBUG then begin 
              Printf.eprintf "tableau avec index à terminer\n";(* afficherAS a *) 
...
let expressionAffectVar v (*index*) l =
  match rechercheAffectVDsListeAS v (*index*) l with 
    MULTIPLE -> NOTHING 
  | EXP (e) -> e
...
let rec rechercheLesVar listeaS listeVAR =
  if (listeaS = []) then 
    ...
    match aSCourant with
      ASSIGN_SIMPLE (id, _) ->
        if(List.mem id listeVAR) then 
          (rechercheLesVar suite listeVAR)
        else  
          List.append (rechercheLesVar suite listeVAR) [id]
    | ASSIGN_DOUBLE (id, _, _) ->
        if(List.mem id listeVAR) then 
          (rechercheLesVar suite listeVAR)
        else 
...
(* jz: find all the assignments that assign function pointers
       this will be used on the asLs that we generate for
       ifs and loops (i guess)
*)
(* FIXME: remove, not needed i guess *)
let rec searchFptrAsgns listeaS listOfFcts =
  if (listeaS = []) then 
    ()
  else begin
    let aSCourant = List.hd listeaS in
    let suite = List.tl listeaS in
    match aSCourant with
      ASSIGN_SIMPLE (id, EXP (UNARY (ADDROF, VARIABLE couldBeFct))) ->
        if(List.mem couldBeFct listOfFcts) then begin
        (* FIXME REMOVE *)
(*          (Hashtbl.add varsPointingToFcts id couldBeFct);*)
          Printf.printf "add: %s - %s\n" id couldBeFct
        end ;
        searchFptrAsgns suite listOfFcts
    | _ -> 
        searchFptrAsgns suite listOfFcts
  end
...
let ro var liste =
  List.find (fun aSCourant ->
    match aSCourant with 
      ASSIGN_SIMPLE (id, _) 
    | ASSIGN_DOUBLE (id, _, _)
    | ASSIGN_MEM (id, _, _) -> id = var 
  ) liste

let rofilter var liste =
  List.filter (fun aSCourant -> 
    match aSCourant with 
      ASSIGN_SIMPLE (id, _)
    | ASSIGN_DOUBLE (id, _, _)
    | ASSIGN_MEM (id, _, _) -> 
...
let rec roavant (liste: abstractStore list ) (aS: abstractStore) (resaux : abstractStore list ) =
  if liste = [] then 
    (resaux, [])
  else begin
    ...
    if (aS = head) then
      ...
      (roavant others aS (List.append resaux [head]))
  end
...
let existeAffectationVarIndexListe var l i=
    List.exists (
      fun aS ->  match aS with 
                   ASSIGN_SIMPLE (id, _) -> false 
                 | ASSIGN_DOUBLE (id, index, _) -> (id = var) && eqindex index i
                 | ASSIGN_MEM (id, e, _)  -> eqmemindex id var i e) l
...
*)
            ...
            |_->  
              Printf.printf "traiterChampOfstruct MEMBEROFPTRcas 3 %s non traité\n" id;
              print_expression assignedValue 0;
              remplacerValPar  id  assignedValue e
        )
...
and alldefinedtabMinMax name idmin idmax arrayas = (* (alldedined, min max) *)
  ...
  begin
    ...
    if tabvalueExp = NOTHING then 
      (false, NOCOMP, NOCOMP)
    else begin
      let tabvalueEval = (calculer (EXP(tabvalueExp)) !infoaffichNull  [] 1 )in
      if estNoComp tabvalueEval || estDefExp tabvalueEval = false then 
        (false, NOCOMP, NOCOMP)
      else 
        if idmin = idmax then
          (true, tabvalueEval,tabvalueEval)
        else 
          alldefinedtabMinMaxNext name (idmin +1) idmax arrayas tabvalueEval tabvalueEval
    end
  end
...
let getVectorAndNumberOfTerms exp lv =
  ...
  | v1::lvn ->
    ...
    if (estAffine v1 newexp1) then begin 
      let (a, b) = calculaetbAffineForne v1 newexp1 in    
      let var1 = evalexpression a in
      if estDefExp var1 then begin
        let val1 = getDefValue var1 in
        if val1 > 0.0 then
          (nb, (v1, val1)::getVector exp lvn, hasconstant)
        else begin
          if (val1 = 0.0) then  
            (nb, getVector exp lvn, hasconstant)
          else (nb, (v1, val1)::getVector exp lvn, hasconstant)  
        end
      end else
        (0, [], false)
    end else
...
let rec filterGlobalesAndOthers ascour globales =
  if ascour = [] then
    ...
    (match prem with
      ASSIGN_SIMPLE (x, _) -> 
        if (List.mem x globales ) then
          (List.append [prem] nextGlobal, nextOthers)
        else 
          (nextGlobal, List.append [prem] nextOthers)
    | ASSIGN_DOUBLE (x, i, assign)
    | ASSIGN_MEM (x, i, assign) ->
        if (List.mem x globales ) then begin
          (List.append [prem] nextGlobal, nextOthers)
        end else begin
          let fid = 
          ...
          in
          if List.mem_assoc fid !listeAssosPtrNameType then begin
            let listeDesVar = listeDesVarsDeExpSeules (expVaToExp (i)) in
            let vdij = (intersection listeDesVar globales) in
            if vdij = [] then
              (nextGlobal, List.append [prem] nextOthers)
...
let absMoinsTEm x a1 a2 listeT =
  let res =
    if ( existeAffectationVarListe x a1) then begin
      let ro1 = ro x a1 in
    if (existeAffectationVarListe x a2) then begin
      let (r,_) = rondListe a1 ( ro x a2 ) in
      let ro2 = if r= [] then ro1 else  List.hd r in
      if ( List.mem x listeT) then 
        ro2 (*changed into the two alternatives*)
      else begin
        if ro1 = ro2 then
          ro1 (*changed into the only one alternative*)
        else begin
    ...
    end else (* rien pour x dans a2 on garde ce qu'il y a dans a1 *)
      ro1
  end else begin
    let (r,_) = rondListe a1 (ro x a2) in
    ...
    begin
      let resb =
        ...
        end else if (String.length x > 3) then 
...
let filterIF l =
  List.filter (
    fun e -> 
      match e with 
        ASSIGN_SIMPLE (id, _) ->
          if (String.length id > 3) then begin
            if (String.sub id 0 3) = "ET-" || (String.sub id 0 3) = "EF-" then
              true
            else 
              false
          end else (* if existAssosPtrNameType  id then true else*)
            false
      | _-> false 
  ) l
...
let filterwithoutWH l =
  ...
  List.filter (
    fun e -> 
      match e with
        ASSIGN_SIMPLE (id, _) ->
          if (String.length id > 4) then begin
            if (String.sub id  0 4) = "TWH-" then
              if !firstWH then
                (firstWH := false; true) 
              else 
                false
            ...
            EXP(CONSTANT(CONST_COMPOUND(_)))->
              ...
              let isD = LocalAPContext.alwayget ptr id in
                if MyPtrDomain.is_refer isD then
                  (MyPtrDomain.getVal isD, MyPtrDomain.getRef isD)
                else 
                  ([],["__extern"] )
              in
              ...
              let gnen =
                if ptrvalues !=[] then
                  mayBeAssignVar (List.filter (fun x-> List.mem x !listEnumId = false && 
                                  List.mem_assoc x !listeAssosPtrNameType = false)
                                  !alreadyAffectedGlobales) t
                else
                  []
              in
              let (x, onlyOne ) = 
                if ptrvalues = [] && values != [] && List.tl values = [] then 
                  (List.hd values, true)
                else
                  ("", false)
                in
              let listToAdd =
                if onlyOne (*&& List.mem x u = false*) then
                  if existAssosArrayType x = false then 
                    [new_assign_simple x exp] 
                  else [new_assign_double x (EXP(VARIABLE(id))) exp]
                else 
                  makeListOfMem u values ptrvalues gnen (EXP(VARIABLE(id))) exp id
              in
              listToAdd
      ...
      let addList =
        if isPtr fid then
          if ptr != [] then begin
            ...
            in
            let t = 
              if existAssosPtrNameType fid then 
                getAssosPtrNameType fid 
              else
                INT_TYPE
            in
            let gnen = 
              if ptrvalues != [] then
                mayBeAssignVar (List.filter
                    (fun x-> List.mem x !listEnumId = false &&
                             List.mem_assoc x !listeAssosPtrNameType = false)
                    !alreadyAffectedGlobales) t
              else
                []
            ...
            let (x, onlyOne ) =
              if ptrvalues = [] && values != [] && List.tl values = [] then 
                (List.hd values, true) else ("", false) 
            in
            let listToAdd =
              if onlyOne  (*&& List.mem x u = false*) then
                if existAssosArrayType x = false then
                  [new_assign_simple x exp2]
                else
                  [new_assign_double x exp1 exp2]
          ...
          if ptr != [] then begin
            ...
            let listToAdd =
              if onlyOne then (* on sait quelle est la variable modifiée et aucun autre pointeur peut la changer donc on peut rempalcer*)
                ...
                makeListOfMem u values ptrvalues gnen exp1 exp2 fid
            in
            listToAdd
            (*Printf.printf "%s  : [" fid ;List.iter(fun x -> Printf.printf "%s  "x) vdij;Printf.printf "]\n" );*)
          end else
            []
        else
          []
...
and getNextaa nomFonc entrees used newptr body s ptr isAbs =
  ...
  if outputMem != [] || globalMem != [] then begin
...
and getLinkedVarByCathegorie l =
  listWithoutv l "__extern"
...
and listOfMaybechangedVarWithout l v =
  (*let _= listOfMaybechangedVar l in let l2 = List.filter (fun a->let (x,_)= a in x != v) in let _= listOfMaybechangedVar l2 in*)
  if l = [] then
    []
  else begin
    let (f, n) = (List.hd l, List.tl l) in
...
and getMemAssign sortie a g i ptr=
  match sortie with
    MEMASSIGN (id, e1, e2, _, _) ->
    (* Printf.printf "getMemAssign ...ok %s \n" id;let index = applyStoreVA(applyStoreVA e1 a)g in print_expVA index; new_line();
afficherListeAS a;*)
      (match e2 with
...
and notchangeSortie var a =
  ...
  let res =
    if (existeAffectationVarListe ("*"^var) a) then
      false
    else if existassigndouvle var a then false else true
  in
...
and getParamPtr entrees =
  if entrees <> [] then begin
    ...
    match entree with
      VAR (id, exp,_,_) -> 
        if existAssosPtrNameType id then 
          id::(getParamPtr others)
        else
          (getParamPtr others)
    | MEMASSIGN ( id, _, _,_,_) ->
        if existAssosPtrNameType id then 
    ...
    fun asc ->
      match asc with
        ASSIGN_SIMPLE (id, e) ->
          ...
          if na != asc then begin
            if (String.length id > 4) then begin
              let var3 = (String.sub id 0 3) in
              if var3 = "IF-"|| var3 = "ET-" || var3 = "EF-" then
                asc
              else
                na
            end else
...
and rewriteAllOthers others isexe=
  if others = [] then 
    ...
    List.append (rewriteOthers (List.hd others) isexe) (rewriteAllOthers (List.tl others) isexe)
      ...
      match assign with
        ASSIGN_SIMPLE (id, e) ->
          if (String.length id > 4) then begin
            let var4 = (String.sub id  0 4) in
            let var3 = (String.sub id  0 3) in
            if var3 = "IF-" || var3 = "ET-" || var3 = "EF-" || var3 = "tN-" || 
               var4 = "max-" || var4 = "tni-" || var4 = "TWH-" then 
                 false 
            else
              true
          end else if (String.length id > 3) then 
            if (String.sub id 0 3) = "IF-" || (String.sub id 0 3) = "ET-" ||
               (String.sub id  0 3) = "EF-" || (String.sub id  0 3) = "tN-" then 
                 false 
            else
              true
          else
...
and rewriteOthers assign isexe =
  match assign with
    ASSIGN_SIMPLE (id, e) ->
      ...
      let na = 
        if isexe then 
          ASSIGN_SIMPLE (id, e)
        else ASSIGN_SIMPLE (id, EXP(CALL (VARIABLE("SET"), List.append [VARIABLE(id)] [expVaToExp e]))) 
      in
      if (String.length id > 4) then begin
        let var4 = (String.sub id  0 4) in
        let var3 = (String.sub id  0 3) in
  ...
  | ASSIGN_MEM (id, e1, e2) ->
      ...
      if isTruecteArg1 then begin
        ...
        [ASSIGN_MEM (id, e1, MULTIPLE)]


and closeFormrec id l listeaS listeDesVarModifiees =
  (*Printf.printf "closeFormrec %s \n" id;*)
  if (listeaS = []) then begin 
    (*Printf.printf "FIN closeFormrec %s \n" id; *)
    ([], [])
  end else begin
    let aSCourant = List.hd listeaS in
    let suite = List.tl listeaS in
...
and remplacerUneAffect assign aSC =
  ...
  | ASSIGN_DOUBLE (id, e1, e2) ->
      (match e1 with
        MULTIPLE -> assign
      | EXP (exp) ->
          if (intersection (listeDesVarsDeExpSeules exp) listeDesVarModifiees ) != [] then begin
            (match e2 with
              MULTIPLE -> assign
            | EXP (exp2) ->
...
and remplacerToutes reverseliste l listeaffectEtape  =
  ...
  if listeaffectEtape = [] then begin (*Printf.printf "liste affect etape vide\n";*)
    ...
    if teteaux = [] then
      (List.append [derniereAffectCour] (remplacerToutes (List.tl reverseliste) suite (List.tl listeaffectEtape)))
...
let applyif exp a =
  if a = [] || exp = MULTIPLE then 
    exp
  else begin
    let varOfExp1 = listeDesVarsDeExpSeules (expVaToExp exp) in
    let na = 
      List.filter (
        fun ass -> match ass with
          ASSIGN_SIMPLE (id, _)
        | ASSIGN_DOUBLE (id, _, _)
        | ASSIGN_MEM (id, _, _) ->
            if List.mem id varOfExp1 && listVARMAXSYG id (expVaToExp exp) then
              false
            else 
              true
      ) a
...
(* fixed point operator to find any useful variable for loop condition*)
...
let rec extractVarCONDAffectForIncaux listeAffect listeCondVar =
  if listeAffect = [] then
    ...
    match affect with
      VAR (id, exp, l, u) ->
        if List.mem id listeCondVar then begin
    ...
    | IFVF ( expVA1, i1, i2) ->
        ...
        if newi1 = [] && newi2 = [] then
          ...
          let unionOfAll = 
            match expVA1 with 
              EXP(VARIABLE(varIfN)) ->
                 union [varIfN] (union (union listeaux1 listeaux2) listeaux)
            | _->
                union (union listeaux1 listeaux2) listeaux in
    ...
    | IFV ( expVA1, i1) ->
        ...
        if newi1 = [] then
          ...
          (let unionOfAll =
            match expVA1 with
              EXP(VARIABLE(varIfN)) -> 
                union [varIfN] (union listeaux1 listeaux)
            | _-> union listeaux1 listeaux in
...
let rec extractVarCONDAffectForInc listeAffect listeCondVar =
  ...
  if listevar = [] || (inclus listevar listeCondVar) then 
    extractsInst
